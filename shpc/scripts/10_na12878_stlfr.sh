...
#SBATCH --output=./logs/na12878_stlfr.%j.out
#SBATCH --error=./logs/na12878_stlfr.%j.err
#SBATCH --mem=450G
#SBATCH --ntasks=1
...
# NA12878 stLFR: Build physical map and scaffold draft assemblies.
#
# Input: stLFR linked reads (barcode in read name after #)
# Drafts: ONT Shasta, PacBio Peregrine, PE+MPET ABySS, stLFR Supernova
#
# Usage: sbatch scripts/10_na12878_stlfr.sh

BASEDIR="/projects/site/dia/sbx/workspace/afshinfa/flexdev/physlr/rust/test1"
source "${BASEDIR}/scripts/common.sh"
mkdir -p "${BASEDIR}/logs"

SAMPLE="na12878_stlfr"
OUTDIR="${BASEDIR}/output/${SAMPLE}"
INPUT_FQ="${DATADIR}/na12878_stlfr/na12878.stlfr.R1.fq.gz ${DATADIR}/na12878_stlfr/na12878.stlfr.R2.fq.gz"

# stLFR parameters (from original Physlr paper)
K=40
W=32
OVERLAP_PCT=85
MOL_STRATEGY="bc+cc"
...
DRAFTS=(
    "na12878.ont.baseline.fa:ont"
    "na12878.pacbio.baseline.fa:pacbio"
    "na12878.pempet.baseline.fa:pempet"
    "na12878.stlfr.baseline.fa:supernova"
)
...
# Evaluate scaffolded assemblies
for entry in "${DRAFTS[@]}"; do
    draft_label="${entry##*:}"
