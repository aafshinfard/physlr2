#!/bin/bash
#SBATCH --job-name=full-na12878
#SBATCH --output=./logs/full_na12878.%j.out
#SBATCH --error=./logs/full_na12878.%j.err
#SBATCH --mem=200G
#SBATCH --ntasks=1
...
OUTDIR="${BASEDIR}/output/na12878_stlfr_final"
...
echo "============================================================"
echo "physlr-next full pipeline (k3 fix + split-minimizers) — NA12878"
...
if [ -f "${REF}" ]; then
    ...
    echo ""
    echo "=== Step 9a: Map backbone to reference (barcode minimizers) ==="
    START=$(date +%s)
    ${PHYSLR} -v2 map-paf \
    ...
    echo ""
    echo "=== Step 9b: Map backbone to reference (split minimizers) ==="
    START=$(date +%s)
    ${PHYSLR} -v2 map-paf \
        sample.backbone.path \
        sample.split.tsv \
