...
pub fn run_ntcard(
    ...
    let status = cmd
        .status()
        .with_context(|| "Failed to run ntcard")?;
    ...
    let actual_path = format!(
        "{}_k{}",
        output_histogram.to_str().unwrap(),
        k
    );
    if Path::new(&actual_path).exists() && !output_histogram.exists() {
        std::fs::rename(&actual_path, output_histogram)
            .with_context(|| format!("Failed to rename {} to {:?}", actual_path, output_histogram))?;
    }
...
pub fn run_nthits(
    ...
    let status = cmd
        .status()
        .with_context(|| "Failed to run nthits")?;
...
pub fn run_indexlr(
    ...
    let child = cmd
        .spawn()
        .with_context(|| "Failed to spawn indexlr")?;
